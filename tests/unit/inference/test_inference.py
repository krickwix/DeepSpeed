...
def invalid_model_task_config(model_w_task, dtype, enable_cuda_graph):
    ...
    elif (not bool(pytest.use_hpu) == True) and enable_cuda_graph and (torch_info["cuda_version"] == "0.0"):
...
class TestModelTask(DistributedTest):
    ...
    def test(
        ...
        if dtype == torch.bfloat16:
            pipe = pipeline(task, model=model, device=-1, framework="pt", torch_dtype=torch.bfloat16)
        else:
...
class TestMPSize(DistributedTest):
    ...
    def test(
        ...
        if bool(pytest.use_hpu) == True:
            ...
            pipe.device = torch.device(f"hpu:{local_rank}")
        else:
